...
class GnssDateLocBuilder:
    """Build GNSS date information for the assisted sample."""

    EPOCH_TIME = datetime.datetime(1980, 1, 6)

    def __init__(self):
        """Initialize the GNSS date helper."""
        pass
    ...
    def date_to_gps_format(date):
        """Convert a datetime value to GPS seconds since the GPS epoch."""
        delta_since_epoch = date - GnssDateLocBuilder.EPOCH_TIME
        return int(delta_since_epoch.total_seconds())

    @staticmethod
    def get_now_gps():
        """Return the current UTC time represented in GPS seconds."""
        today = (
