"""This python script will take nav payload from lr11xx chip, assistance location and time in gps format. It will make post request to lora cloud server and in case of correct parameters it will return location.
...
parser = argparse.ArgumentParser(
    description=intro, formatter_class=RawTextHelpFormatter
)
parser.add_argument(
    "--nav",
    type=str,
    required=True,
    help="This is nav payload, make sure that is in hexadecimal form",
)
parser.add_argument(
    "--lat",
    type=str,
    required=True,
    help="This is latitude coordinate of assistance position",
)
parser.add_argument(
    "--lon",
    type=str,
    required=True,
    help="This is longitude coordinate of assistance position",
)
parser.add_argument(
    "--gpstime", type=str, required=True, help="This is GSP time in epoch format"
)
...
response = requests.post(
    ...
    headers={
        ...
        "Ocp-Apim-Subscription-Key": token.AUTH_TOKEN,
    },
)
print()
...
if json_content["result"] is None:
