...
uint8_t S_Table[16][16] = {{0x63, 0x7C, 0x77, 0x7B, 0xF2, 0x6B, 0x6F, 0xC5, 0x30, 0x01, 0x67, 0x2B,
			    0xFE, 0xD7, 0xAB, 0x76},
...
void AES_Encrypt(uint8_t *Data, uint8_t *Key)
{
	...
	for (Column = 0; Column < 4; Column++) {
		for (Row = 0; Row < 4; Row++) {
			State[Row][Column] = Data[Row + (4 * Column)];
		}
	}
	...
	for (Column = 0; Column < 4; Column++) {
		for (Row = 0; Row < 4; Row++) {
			Data[Row + (4 * Column)] = State[Row][Column];
		}
	}
}
...
void AES_Add_Round_Key(uint8_t *Round_Key)
{
	...
	for (Column = 0; Column < 4; Column++) {
		for (Row = 0; Row < 4; Row++) {
			State[Row][Column] = State[Row][Column] ^ Round_Key[Row + (4 * Column)];
		}
	}
}
...
{
	uint8_t S_Row, S_Column;
	uint8_t S_Byte;

	// Split byte up in Row and Column
	S_Row = ((Byte >> 4) & 0x0F);
	S_Column = (Byte & 0x0F);
...
/*
*****************************************************************************************
* Description : Function that preforms the Mix Columns operation described in the AES standard
*****************************************************************************************
*/
